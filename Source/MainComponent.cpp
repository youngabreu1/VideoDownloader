...
void MainComponent::runYtDlpCommand(juce::String url, juce::String path, bool isAudioOnly, juce::String heightLimit)
{
    ...
    if (!ytDlpExe.exists())
    {
        statusLabel.setText("ERRO: yt-dlp.exe nao existe!", juce::dontSendNotification);
        return;
    }

    if (isAudioOnly && !ffmpegExe.exists())
    {
        statusLabel.setText("ERRO: ffmpeg.exe faltando. Nao e possivel converter para MP3.", juce::dontSendNotification);
        return;
    }
